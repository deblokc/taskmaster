use crate::{Log, Program, RestartState};
use std::{
    collections::HashMap,
    convert::TryFrom,
    io::Error,
            ...
            match command.spawn() {
                ...
                Err(er) => {
                    if self.count_restart < program.startretries {
                        ...
                        Log::log(Log::INFO, format!("restart: {} (cause : {er})", self.name));
                        continue;
                    } else {
    ...
    if let Ok(mut proc) = proc_ref.lock() {
        ...
        // if process wasnt already down
        {
            if matches!(proc.status, Status::STOPPING) {
                ...
                Log::log(Log::INFO, format!("status: {} STOPPED", proc.name));
            }
            if matches!(proc.status, Status::STARTING) {
                // if process was starting
                proc.status = Status::BACKOFF;
                Log::log(Log::INFO, format!("status: {} BACKOFF", proc.name));
            }
        }
        Log::log(
            Log::DEBG,
        ...
        match process_handle.try_wait() {
            ...
            Ok(Some(status)) => {
                ...
                *exit_status = Some(status);
                Log::log(Log::INFO, format!("exited: {name} ({status})"));
            ...
            Err(e) => {
                ...
                Log::log(Log::ERRO, format!("try_wait failed : {e}"));
            }
        }
    }
}
