...
/*   Updated: 2023/09/20 14:44:02 by tnaton           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void wait_process(struct s_program *lst)
{
	for (int i = 0; lst; i++, lst = lst->next)
	{
		...
		if (lst->numprocs == 1)
		{
			...
			lst->processes[0].stdout_logger.logfile = NULL;
			lst->processes[0].stderr_logger.logfile = NULL;
			lst->processes[0].name = NULL;
			if (lst->stdout_logger.logfile)
				lst->stdout_logger.logfile = NULL;
			if (lst->stderr_logger.logfile)
				lst->stderr_logger.logfile = NULL;
			free(lst->processes);
			lst->processes = NULL;
		}
		else
